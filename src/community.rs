...
pub fn collect_public_cat_cards(
    ...
    for email in emails {
        ...
        let breed_slug = breed_slug_for_name(&profile.pet_breed);
...
pub fn render_cat_feed_card(card: &PublicCatCard) -> String {
  ...
  <div class="community-cat-photo-wrap">{photo}</div>
  <h3 class="community-cat-name">{name}</h3>
  ...
  <p class="community-cat-parent">by {parent}</p>
        ...
        photo = photo,
        name = escape_html(&card.pet_name),
...
pub fn render_cat_feed_section(
    ...
    let grid = if cards.is_empty() {
        if profile_has_pet(profile) {
            r#"<p class="community-feed-empty">No other cats match this filter yet. Invite friends or check back as the community grows!</p>"#
                .to_string()
        } else {
            r#"<p class="community-feed-empty">Create your pet profile to join the community feed and see other WhiskerWatch cats.</p>"#
                .to_string()
        }
    } else {
        format!(
  ...
  <div class="community-section-header">
    ...
    <p class="field-hint">A public showcase of cats from other WhiskerWatch parents. Hide yours anytime on the Account tab.</p>
  </div>
